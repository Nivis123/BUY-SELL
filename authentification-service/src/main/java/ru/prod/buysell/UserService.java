...
public class UserService {
    ...
    @Transactional
    public void createPendingUser(UserRegistrationRequest request) {
        if (userRepository.findByEmail(request.getEmail()) != null) {
            throw new BusinessException("Пользователь с email " + request.getEmail() + " уже существует");
        }
        User user = userMapper.toEntity(request);
        ...
        user.getRoles().add(Role.ROLE_USER);
        userRepository.save(user);
        log.info("Pending user created. Email: {}", request.getEmail());
    }
    ...
    public void activateUser(String email) {
        ...
        if (user == null) {
            throw new BusinessException("Пользователь с email " + email + " не найден");
        }
        if (!user.isActive()) {
            user.setActive(true);
            userRepository.save(user);
